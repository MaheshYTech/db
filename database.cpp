...
void printRecord(void* data, char ftype[], int foffset[], int record_indx) {
    ...
    for (int i = 0; i < dbmain.field_count; i++) {
        ...
        case 'F':
            //printf("%-10.2f", *(float*)(data + foffset[i])); // seleFields[i].Fields.foffset));
            //sprintf(field_format, "\t%%-%ds", dbmain.fwidth[i]);
            //printf(field_format, ((char*)data + foffset[i]));
